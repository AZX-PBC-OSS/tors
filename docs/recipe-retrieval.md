...
This is a **lexical, statistical** pipeline (SimHash, TF-IDF, BM25,
substring/fuzzy matching): no embeddings, no neural ranking, no semantic
understanding of either the query or the documents. It is the "fast, no GPU,
no API key, good enough for tens to a few hundred documents" tier, not a
claim that it beats an embedding-based retrieval system on relevance. Reach
for one of those when this tier's quality isn't enough; reach for this when
it is.
...
Two chunkers producing overlapping windows, or a corpus with repeated
content, both leave near-identical chunks behind. `simhash64` gives
...
question, exact equality per index rather than similarity:
...
actually supported by the source text: a substring/fuzzy-match check, not a
