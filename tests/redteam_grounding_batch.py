...
attack; the three red attacks (one P0, two P1s, each xfail(strict)) are
green pins in this suite; the findings list below records which, and how.
...
Findings pinned by the tests in this file:

- P0, GREEN PIN: CJK-range PUNCTUATION (U+30FB katakana middle dot, U+3099)
  is token-free under both tokenizer branches: the documented "segments with
  no alphanumeric character are dropped" rule applies to the CJK sub-split's
  flushed runs too, so token-free operands score exactly 0.0, never 1.0.
  The project's own fuzz target (fuzz_targets/grounding_coverage.rs) crashes
  on this cell (artifact crash-094c9dae62d4a55cddba683a1444a2be910dc0f3);
  120s on ground_sentences come up empty.
- P1, GREEN PIN: the core's scores are NOT Lin 2004 ROUGE-W: the published
  forced-diagonal fill disagrees with the max-on-match spelling on ~12% of
  random token pairs, so tors scores are not comparable with the official
  ROUGE package / rouge-score.  The deviation is DELIBERATE (the forced
  diagonal is not candidate-monotone) and DISCLOSED on every user-facing
  surface; the cell pins the documented max-on-match recurrence as a green
  test.
- P1, GREEN PIN: the module-doc claim that the max-on-match spelling
  ...
  (Pareto-stranded runs the two-row DP cannot represent).  The docs say
  "max-on-match recurrence (a greedy-run-weighted alignment score), not the
  literal weighted-LCS optimum"; the cell pins that documented semantics as
  a green test.
- GREEN: candidate monotonicity (extending the text never lowers
  grounding_coverage), the property the change buys, holds under Hypothesis
  attack; the core matches an independent re-derivation
  of ITS OWN documented recurrence to 1e-9 on hundreds of shapes; the
  degenerate conventions hold for every token-free shape outside the P0
  finding above; sentence spans are EXACTLY ``sentence_bounds`` tuples on
