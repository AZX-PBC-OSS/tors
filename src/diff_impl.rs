...
//! `a[i1:i2]` is the slice an op describes. Character level is deliberate:
...
//! consequences, both deliberate and pinned on the Python side
...
//! the honest measured shape — the bounded search's work grows ~n^2 with
...
// a famous pain point. The honesty crux these docs keep front and center:
// difflib's ratio is 2.0*M/T over ITS OWN anchored alignment (M the sum of
// the matching blocks' sizes, difflib.py:597-620) — an anchoring-dependent
// number, NOT a canonical metric — so this surface's contract is
...
// difflib parity pinned only on the forced-alignment classes — identical
// operands (1.0), the empty-pair classes, disjoint alphabets (0.0), pure
// insert/delete with differing flanks — and the divergent repeated-flank
// rows pinned as divergent (both numbers valid, both in [0,1]). Every
...
/// with the zero-total guard returning `1.0` — difflib's own answer for the
/// empty pair (`SequenceMatcher(None, "", "").ratio() == 1.0`,
...
/// and empty vs nonempty is `0.0` (`M = 0`, `T > 0`). Identical operands
...
/// divergence rows: `"ppp"` vs `"pwpp"` — difflib anchors `"pp"` and
/// starves the flank (`M = 2` → `4/7` ≈ 0.5714) where the Myers one-insert
/// script matches `1 + 2 = 3` → `6/7` — and `"qpqpq"` vs `"qpwqpq"` —
/// difflib anchors `"qpq"` (`M = 3` → `6/11` ≈ 0.5455) where
...
/// each candidate's search — a budget spent mid-list STOPS the scan, the
/// partial result list is DISCARDED, and [`DeadlineExceeded`] is returned
...
pub fn close_matches(
    ...
    for (idx, candidate) in candidates.iter().enumerate() {
        ...
        if let Some(err) = exceeded_after(started, deadline_ms) {
            // A budget spent mid-list stops the scan; the partial result
            // list is discarded — never a silently truncated answer.
            return Err(err);
        }
    ...
    // difflib's order (difflib.py:707, difflib.py:710): heapq.nlargest over
    // the (score, candidate) tuples — score descending, then the candidate
    // itself descending, exact duplicates stable in input order.
    ...
    scored.truncate(n);
    Ok(scored.into_iter().map(|(_, idx)| idx).collect())
}
...
mod tests {
    ...
    fn the_boundary_case_opcodes_are_pinned() {
        ...
        // slid to one side): "a" vs "baa" — the anchored middle 'a' splits
        // the insertion — and "ppp" vs "pwpp" (plus its delete-class twin)
        // — the equal-run boundary slides across a repeated-flank insertion
    ...
    fn split_keepend_lines_diverges_from_str_splitlines_on_non_lf_terminators() {
        // The pinned, deliberate narrower-than-splitlines contract (see the
        ...
        // this behavior is a deliberate, visible diff, not silent.
        assert_eq!(
    ...
    fn ratio_pins_the_forced_alignment_classes() {
        ...
        // The empty pair: difflib's own answer is 1.0 — _calculate_ratio's
    ...
    fn the_ratio_boundary_rows_where_difflibs_anchored_m_diverges() {
        // The honesty rows, the ratio twin of the opcode boundary pins:
